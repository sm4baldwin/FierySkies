  ...
  async (userInfo: {username: string, pass: string}, {dispatch, rejectWithValue}) => {
    try {
      ...
      if (firestoreDocSnap.exists()) {
        ...
        await setDoc(firestoreDocRef, {username: userInfo.username, password: userInfo.pass, connectionStatus: 'online', activeLobby: '', playerTag: ''})
        const realtimeDBuserRef = ref(realtimeDB, 'users/' + userInfo.username)
...
export const startGame = createAsyncThunk<any, any, {dispatch: AppDispatch, state: RootState}>(
  ...
  async (_, {dispatch, getState}) => {
    const state = getState()
    const roomID = state.database.data.room.id ? state.database.data.room.id : 'no active room'
...
export const databaseSlice = createSlice({
  ...
  reducers: {
    ...
    updateRoomGameID: (state, action: PayloadAction<string>) => {
      state.data.room.gameID = action.payload
    }
  },
...
export const { updateRoomPlayers, updateRoomViewers, updateRoomGameID } = databaseSlice.actions
...
export const startRoomListener = (): AppThunk => (dispatch, getState) => {
  ...
  const unsubscribe = onSnapshot(doc(firestore, "lobbies", roomID ? roomID : ''), (doc) => {
    ...
    if (roomData && roomData.gameID) {
      ...
      dispatch(updateRoomGameID(roomData.gameID))
    }
